"""
Preprocessing for per-sequence embeddings.
Uses doc_embeddings with 'embedding_per_sequence' (one 128-dim embedding per 30-second sequence).
"""
import numpy as np
import os
import h5py
...
def main():
    parser = argparse.ArgumentParser(description='Preprocess per-sequence embeddings')
    parser.add_argument('--main_path', type=str, 
                        default='/home/triniborrell/home/projects/sleepfm-clinical/sleepfm/pretrained_model/doc_embeddings',
                        help='Path to doc_embeddings folder')
    parser.add_argument('--output_path', type=str, default='./data_per_sequence', 
    ...
    for file in os.listdir(args.main_path):
        if file.endswith('.hdf5'):
            ...
            with h5py.File(file_path, 'r') as f:
                ...
                if 'embedding_per_sequence' not in f:
                    print(f"Warning: 'embedding_per_sequence' not found in {file}, skipping...")
                    continue
                
                data = f['embedding_per_sequence'][:]  # Shape: (n_sequences, 128)
                print(f"  Shape: {data.shape} ({data.shape[0]} sequences)")
                
                # Add each sequence as a separate row
                for seq_idx in range(data.shape[0]):
                    patient_ids.append(patient_id)
                    labels_list.append(patient_label)
                    sequence_ids.append(seq_idx)
                    embeddings_list.append(data[seq_idx])
    
    # Create DataFrame
    csv_data = pd.DataFrame(embeddings_list)
    csv_data.insert(0, 'patient_id', patient_ids)
    csv_data.insert(1, 'sequence_id', sequence_ids)
    csv_data.insert(2, 'diagnostic_label', labels_list)
    
    output_file = os.path.join(args.output_path, 'embeddings_per_sequence.csv')
