"""
Training pipeline for per-sequence embeddings.
...
def evaluate_model(model, X_test, y_test, groups_test, output_path):
    ...
    plt.tight_layout()
    plot_path = os.path.join(output_path, 'evaluation_plots_per_sequence.png')
    plt.savefig(plot_path, dpi=150, bbox_inches='tight')
    plt.close()
    print(f"\nPlots saved to {plot_path}")
    
    # Save metrics to file
    metrics_path = os.path.join(output_path, 'metrics_per_sequence.txt')
    with open(metrics_path, 'w') as f:
        f.write("TEST SET EVALUATION METRICS (Per-Sequence Model)\n")
        f.write("="*50 + "\n")
        for metric, value in metrics.items():
        ...
        f.write(f"Test sequences: {len(y_test)}\n")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Train per-sequence ML model')
    parser.add_argument('--data_path', type=str, 
                        default='/home/triniborrell/home/projects/sleepfm-clinical/sleepfm/ml_model/data_per_sequence/embeddings_per_sequence.csv',
                        help='Path to preprocessed embeddings CSV')
    parser.add_argument('--output_path', type=str, 
                        default='/home/triniborrell/home/projects/sleepfm-clinical/sleepfm/ml_model/trained_model_per_sequence',
    ...
    print(f"After filtering to valid labels {valid_labels}: {len(data)} sequences")
    
    # Extract features, labels, and groups
    id_col = 'patient_id'
    feature_cols = [c for c in data.columns if c not in [id_col, 'sequence_id', 'diagnostic_label']]
    ...
    print(f"  Train+Val: {len(X_trainval)} sequences from {len(groups_trainval.unique())} patients")
    print(f"  Test: {len(X_test)} sequences from {len(groups_test.unique())} patients")
    ...
    # Save model
    model_path = os.path.join(args.output_path, 'random_forest_per_sequence.joblib')
    joblib.dump(best_model, model_path)
    print(f"\nModel saved to {model_path}")
    
    # Save grid search results
    cv_results = pd.DataFrame(grid_search.cv_results_)
    cv_results_path = os.path.join(args.output_path, 'grid_search_results_per_sequence.csv')
    cv_results.to_csv(cv_results_path, index=False)
    print(f"Grid search results saved to {cv_results_path}")
    
    # Save best parameters
    params_path = os.path.join(args.output_path, 'best_params_per_sequence.txt')
    with open(params_path, 'w') as f:
        f.write("Best Parameters from Grid Search (Per-Sequence Model):\n")
        f.write("="*50 + "\n")
